...
const isFormValid = computed(() => {
    return !!opponent.value && !!activity.value && !!time.value && !!scoreUserTeam.value>=0 && !!scoreOpponentTeam.value>=0;
});
...
function postMatch() {
    fetch("http://localhost:3000/matches", {
        ...
        body: JSON.stringify({
            ...
            startedAt: new Date().toISOString().split("T")[0] + "T" + time.value + ":00Z",
        ...
        if (!res.ok) {
            return res.json().then((data) => {
                if (data.message === "Invalid match parameters") {
                    ...
                    errorMessage.value = "Un match contre cet adversaire pour cette activité existe déjà.";
                    console.log(data.message);
                } else {
            ...
            <div class="game-main-container">
                <div class="game-input-container">
                    ...
                    <select name="opponent" id="opponent" v-model="opponent" required>
                        <option value="" disabled selected hidden>Sélectionner une équipe</option>
                ...
                <div class="game-input-container">
                    ...
                    <select name="activity" id="activity" v-model="activity" required>
                        <option value="" disabled selected hidden>Sélectionner une activité</option>
                ...
                <h2>Scores finaux</h2>
                <div class="game-input-container">
                    <p>Mon équipe : <strong>{{ user.team.name }}</strong></p>
                    <input type="number" id="scoreUserTeam" name="scoreUserTeam" v-model="scoreUserTeam" min="0" />
                </div>
                <div class="game-input-container">
                    <p>Adversaire : <strong>{{ selectedOpponentName }}</strong></p>
                    <input type="number" id="scoreOpponentTeam" name="scoreOpponentTeam" v-model="scoreOpponentTeam" min="0" />
                </div>
